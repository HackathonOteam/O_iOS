...
final class SummaryViewController: UIViewController {
    ...
    init() {
        ...
        SummaryService.getSummary(userName) { res in
            self.emotionImageView.image = UIImage(named: res.emotion)
            self.contentLabel.text = res.summary
            
            let components = res.date.split(separator: ".")
